...
def test_category_expense_income_summary():
    # 1) Create category
    r = client.post("/v1/categories/", json={"name": "TestCat", "limit_amount": 100})
    assert r.status_code == 201
    cat_id = r.json()["id"]

    # 2) Expense that exceeds limit
    r = client.post("/v1/expenses/", json={
        "category_id": cat_id,
        "amount": 150,
        "date": "2025-05-10",
    ...
    # 3) Set income
    r = client.post("/v1/income/", json={"month": "2025-05", "amount": 500})
    assert r.status_code == 201

    # 4) Fetch summary
    r = client.get("/v1/summary/2025-05")
...
def test_category_crud_and_404():
    #create
    r = client.post("/v1/categories/",json={"name":"CatA","limit_amount":500})
    assert r.status_code == 201
    cat = r.json()
    cat_id = cat["id"]
    ...
    #get by id
    r = client.get(f"/v1/categories/{cat_id}")
    assert r.status_code==200 and r.json()["id"]==cat_id

    #404 on non existing
    r = client.get("/v1/categories/9999")
    assert r.status_code == 404

    #update
    r = client.put(f"/v1/categories/{cat_id}",json={"limit_amount":750})
    assert r.status_code == 200 and r.json()["limit_amount"]==750

    #delete
    r = client.delete(f"/v1/categories/{cat_id}")
...
def test_expense_crud_and_filter_404():
    #setup a category
    r = client.post("/v1/categories/",json={"name":"CatB","limit_amount":100})
    cid = r.json()["id"]

    #create
    exp = {"category_id":cid,"amount":20,"date":"2025-06-01","description":"T"}
    r = client.post("/v1/expenses/",json=exp)
    assert r.status_code==201
    exp_id = r.json()["id"]

    #404 for bad category
    r = client.post("/v1/expenses/", json={**exp, "category_id":9999})
    assert r.status_code==404

    #read by id
    r = client.get(f"/v1/expenses/{exp_id}")
    assert r.status_code == 200

    #404 for missing
    r = client.get("/v1/expenses/9999")
    assert r.status_code == 404

    #update
    r = client.put(f"/v1/expenses/{exp_id}", json={"amount":30})
    assert r.status_code == 200 and r.json()["amount"]==30

    #filter by month
    r=client.get("/v1/expenses/",params={"month":"2025-06"})
    assert r.status_code==200 and any(e["id"]==exp_id for e in r.json())

    #delete
    r = client.delete(f"/v1/expenses/{exp_id}")
    assert r.status_code == 204
...
def test_income_crud_and_404():
    #create
    r = client.post("/v1/income/",json={"month":"2025-07","amount":2000})
    assert r.status_code == 201 and r.json()["amount"] == 2000

    #update
    r = client.post("/v1/income/",json={"month":"2025-07","amount":2500})
    assert r.status_code == 201 and r.json()["amount"] == 2500

    #read
    r = client.get("/v1/income/2025-07")
    assert r.status_code == 200

    #404
    r = client.get("/v1/income/1999-01")
    assert r.status_code == 404

def test_summary_and_current_month_default():
    
    r = client.post("/v1/categories/",json={"name":"CatC", "limit_amount":200})
    cid = r.json()["id"]
    client.post("/v1/expenses/",json={
        "category_id":cid,
        "amount":60,
        "date":"2025-05-10"
    })
    client.post("/v1/income/", json={"month":"2025-05","amount":800})
...
def test_reset_endpoint_clears_data():
    ...
    r = client.post("/v1/categories/", json={"name":"CatX","limit_amount":10})
    cid = r.json()["id"]
    client.post("/v1/expenses/",json={
        "category_id":cid,
        "amount":5,
    ...
    client.post("/v1/income/",json={"month":"2025-05","amount":100})
    ...
    assert client.get("/v1/expenses/").json() == []
    assert client.get("/v1/income/2025-05").status_code == 404